...
1. [](#improved)
    ...
    * Pages now load on demand from a per-page index instead of one large cache blob that had to be fully unserialized on every request, cutting warm render memory by more than half on a 2,000 page test site and keeping per-request cost flat as sites grow; SQLite powers the index when available with a pure PHP fallback, and sites without either keep the previous behavior automatically (`pages.lazy_index` turns it off).
