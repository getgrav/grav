...
1. [](#improved)
    ...
    * Pages, routes, children lists, sort orders and the taxonomy map now load on demand from a per-page index instead of one large cache blob that had to be fully unserialized on every request, making per-request cost flat as sites grow: a 2,000 page test site renders as fast as a 2 page one and uses a quarter of the memory it did before; SQLite powers the index when available with a pure PHP fallback, and sites without either keep the previous behavior automatically (`pages.lazy_index` turns it off).
