 ...
 *
 * Covers: GHSA-2vcx-h8p2-9pg9 (improper handling of highly compressed data in
 * Installer::unZip()). Before extraction, unZip() now bounds the total
 * uncompressed size, entry count, and directory nesting depth, so a crafted
 * install package can no longer fill the disk / exhaust inodes (CWE-409) or
 * nest deeply enough to overflow the recursive cleanup (CWE-674). Because the
 * check runs *before* extractTo(), a rejected archive leaves nothing on disk.
 *
