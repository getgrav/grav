...
class FormFlashSecurityTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testConstruct_GHSAhmcx_KeepsFlashId(): void
    {
        // The flash id comes from blueprint config, never the request, and flex media identifiers
        // parse it, so it must pass through untouched.
        $flash = new FormFlash([
            'id' => 'tmp://forms/session',
            'session_id' => 'abcdef1234567890',
            'unique_id' => 'abcdef1234567890',
            'form_name' => 'test',
        ]);

        $this->assertSame('tmp://forms/session/abcdef1234567890', $flash->getId());
    }
    ...
    public static function providerGHSAhmcx_TraversalIds(): array
    {
        return [
            ...
            ['../../../shhq', 'parent-dir traversal into the webroot (reported in the wild)'],
            ['..', 'bare parent-dir'],
