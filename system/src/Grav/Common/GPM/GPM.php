...
class GPM extends Iterator
{
     ...
     * `enable` means the package is already installed and must be enabled.
    ...
    public function getDependencies($packages)
    {
        ...
        foreach ($dependencies as $dependency_slug => $dependencyVersionWithOperator) {
            ...
            $dependency = $this->getInstalledPackage($dependency_slug);

            // Check if dependency is installed but disabled.
            if ($this->isPluginInstalled($dependency_slug) && $dependency->isEnabled()) {
            ...
            } elseif ($this->isPluginInstalled($dependency_slug) && !$dependency->isEnabled()) {
                // Add dependency as required it to be "enabled".
                $dependencies[$dependency_slug] = 'enable';
                continue;
            } else {
                $dependencyVersion = $this->calculateVersionNumberFromDependencyVersion($dependencyVersionWithOperator);
