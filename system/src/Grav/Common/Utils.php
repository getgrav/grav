...
abstract class Utils
{
    ...
    /**
     * Parses the `Accept` header and produces an array of acceptable
     * MIME types in preference order. If $cull is true, then only the
     * first instance of each MIME type is returned (you only care about
     * the types themselves and not the parameters).
     *
     * @param bool $cull
     *
     * @return mixed
     */
    public static function parseAccept($cull = true) {
        $result = [];
        $header = $_SERVER['HTTP_ACCEPT'];

        // Break at commas
        $header = explode(',', $header);
        // Look at each entry and isolate paramaters
        $index = 0;
        foreach ($header as $entry) {
            ...
            // The first unit is the mime type
            $first = trim(array_shift($units));
            $type = explode('/', $first);
            $node['mime'] = $first;
            $node['type'] = $type[0];
            $node['subtype'] = $type[1];
            $node['params'] = [];
            $node['params']['index'] = $index++;
            $node['params']['q'] = 1.0;

            // Now isolate paramters
            foreach ($units as $unit) {
                $param = explode('=', $unit);
                $key = trim($param[0]);
                $value = trim($param[1]);
                if ($key === 'q') {
                    $node['params'][$key] = floatval($value);
                } else {
                    $node['params'][$key] = trim($value);
                }
            }
        ...
        // Sort result
        usort($result, array(self, "sortAccept"));

        // Cull duplicate values if requested
        $culled = [];
        $seen = [];
        if ($cull) {
            foreach ($result as $entry) {
                if (! in_array($entry['mime'], $seen)) {
                    $seen[] = $entry['mime'];
                    $culled[] = $entry;
                }
            }
    ...
    /**
     * Sorts the array generated by `parseAccept` into preference order based on the spec.
     *   - q value
     *   - specificity
     *     - use of wildcards
     ...
     *   - order of appearance
     *
     * NOTE: Inequalities are reversed because we want the first element in the resulting
     *       array to be the one with highest priority (descending order).
     *
     * @return mixed
     */
    private static function sortAccept($a, $b) {
        if ($a['params']['q'] === $b['params']['q']) {
            $acount = substr_count($a['mime'], '*');
            $bcount = substr_count($b['mime'], '*');
            if ($acount === $bcount) {
                $acount = count($a['params']);
                $bcount = count($b['params']);
                if ($acount === $bcount) {
                    if ($a['index'] === $b['index']) {
                        return 0;
                    } else {
