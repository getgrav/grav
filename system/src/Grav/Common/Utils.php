...
abstract class Utils
{
     ...
     * MIME types in preference order. This function is not completely
     * spec compliant in that it ignores all parameters other than 'q'
     * and doesn't sort on number of parameters.
     *
     * @return mixed
     */
    public static function parseAccept() {
        ...
        // Break at commas
        $header = explode(',', $_SERVER['HTTP_ACCEPT']);

        // Look at each entry and isolate paramaters
        $result = array_map(function($entry) use ($index) {
            list($mime, $priority) = explode(';', $entry . ';q=1.0');
            $priority = explode('=', trim($priority))[1];
    ...
    /**
     * Sorts the array generated by `parseAccept` into preference order based on the following.
     *   - q value
     *   - specificity
     *     - use of wildcards
     *   - order of appearance
    ...
    private static function sortAccept($a, $b) {
        ...
        $aAny = substr_count($a['mime'], '*');
        $bAny = substr_count($b['mime'], '*');

        if ($aAny !== $bAny) {
            return $aAny > $bAny ? 1 : -1;
        }
